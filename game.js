(() => {
  ...
  const state = {
    ...
    handicap: 'off',
    menuIndex: 0,
    settings: { mute: false, volume: 0.7, language: 'en', announcer: true, roundTimeSeconds: 120, p1HealthBars: 3, p2HealthBars: 3 },
    pvp: false,
    fightMode: 'story',
    lastFightItem: null,
    lastFightMode: 'story',
  ...
  function beginBattleSetup(mode = state.battle.mode) {
    ...
    state.battle.activeSide = mode === 'cpu-cpu' ? 'p2' : 'p1';
    state.battle.activeSlot = 0;
  ...
  function renderStageSelect(spinning=false) {
    ...
    if (wheel) {
      ...
      stageOptions.forEach((stage, i) => {
        ...
        card.style.setProperty('--tx', `${offset * 196}px`);
        card.style.setProperty('--ty', `${absOffset * 14}px`);
        card.style.setProperty('--scale', Math.max(0.74, 1 - absOffset * 0.075).toFixed(2));
        card.style.setProperty('--rot', `${offset * -5}deg`);
        card.style.setProperty('--opacity', Math.max(0, 1 - absOffset * 0.15).toFixed(2));
  ...
  function showGalleryHub(message = 'Choose a gallery section.') {
  ...
  function showGalleryPlaceholder(kind) {
    ...
    showGalleryHub(labels[kind] || 'Choose a gallery section.');
  }
  ...
  if (languageSelect) languageSelect.addEventListener('change', e => { state.settings.language = e.target.value; const note = document.getElementById('languagePlaceholderNote'); if (note) note.textContent = `Language placeholder selected: ${e.target.options[e.target.selectedIndex].text}. Final localization comes later.`; });
  ...
  function startFight(item, mode='story') {
    ...
    const team2 = p2Ids.map((id, idx) => new Fighter(id, idx === 0 ? 720 : 770, -1, { left:'arrowleft', right:'arrowright', jump:'arrowup', light:'1', heavy:'2', special:'3', guard:'arrowdown' }, isStory || isPvPAI || isCpuCpu || isTournament ? true : false));
    ...
    if (isStory || isPvPAI || isTournament) {
  ...
  function setStackedHealth(prefix, fighter) {
    ...
    const bar = document.querySelector(`#${prefix}HpOrange`)?.closest('.triple-health') || document.querySelector(`.fighter-box.${prefix === 'p1' ? 'left' : 'right'} .triple-health`);
    if (!bar) return;
  ...
  function renderPauseContent(tab='home') {
    ...
    if (tab === 'settings') {
      content.innerHTML = `<h3>Quick Settings</h3>
        <ul>
          <li><b>Mode:</b> ${f.chapter || f.mode}</li>
          <li><b>Difficulty:</b> ${difficultyLabel(f.difficulty || 'normal')}</li>
          <li><b>Round:</b> ${f.round}${f.bestOfThree ? ` · P1 ${f.p1Rounds} - ${f.p2Rounds} P2` : ''}</li>
          <li><b>Stage:</b> ${(stageOptions.find(st => st.id === f.stage) || {}).name || f.stage}</li>
          <li><b>Story Assist:</b> ${state.storyAssist ? 'On' : 'Off'}</li>
          <li><b>Handicap:</b> ${state.handicap || 'Off'}</li>
        </ul>
        <p>Full settings remain available from the main menu. This quick pause panel is fight-safe, so it will not break the current match state.</p>`;
    } else if (tab === 'moves') {
      ...
      content.innerHTML = `Press <b>P</b> or <b>Escape</b> to resume. You can check settings, controls, and the move list without leaving the fight.`;
    }
  }
  ...
  function finishMatch(winnerText, p1Won) {
    ...
    flashMessage(`${winnerText}\nMATCH OVER · Press R to rematch or ESC for menu`, 2400);
  }
