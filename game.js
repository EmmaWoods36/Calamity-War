(() => {
  ...
  const state = {
    screen: 'main',
    selected: 'rai',
    battle: { mode: 'pvp-local', type: 'single', format: '1v1', p1: 'rai', p2: 'nico', p1Team: ['rai'], p2Team: ['akira'], stage: 'forest', activeSide: 'p1', activeSlot: 0 },
    stageWheelIndex: 0,
    stageSpinTimer: null,
  ...
  function renderStageSelect(spinning=false) {
    ...
    if (wheel) {
      ...
      stageOptions.forEach((stage, i) => {
        ...
        card.style.setProperty('--tx', `${offset * 205}px`);
        card.style.setProperty('--scale', Math.max(0.74, 1 - absOffset * 0.11).toFixed(2));
        card.style.setProperty('--rot', `${offset * -18}deg`);
