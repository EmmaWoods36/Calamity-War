(() => {
  ...
  const assets = {
    ...
    select: 'assets/character_select.png',
    dojo: 'assets/bg_dojo_ruins.png',
    forest: 'assets/bg_forest.png',
    courtyard: 'assets/bg_fortress_courtyard.png',
    hall: 'assets/bg_fortress_hall.png'
  };
  ...
  document.querySelectorAll('[data-action]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (action === 'story') openStory(0);
      if (action === 'pvp') showScreen('pvp');
      ...
      if (action === 'pvp-ai') startFight({ stage: 'forest', player: state.selected, enemy: 'nico', title: 'PvP: Versus AI', chapter: 'PVP MODE', intro: 'Fight a CPU-controlled rival. Use this to test balance without a second player.' }, 'pvp-ai');
  ...
  window.addEventListener('keydown', e => {
    ...
    state.keys.add(key);
    if (key === 'enter' && state.screen === 'main') openStory(0);
  ...
  class Fighter {
    constructor(id, x, facing, controls, isAI=false) {
      const c = characters[id];
      Object.assign(this, { id, name: c.name, c, x, y: floorY, vx: 0, vy: 0, w: 48, h: 112, facing, controls, isAI, onGround: true, hp: c.hp, maxHp: c.hp, meter: 10, maxMeter: 100, attackTimer: 0, attackKind: null, hitCooldown: 0, guard: false, dead: false, aiCooldown: 0, comboFlash: 0 });
    }
    ...
    input(enemy) {
      ...
      if (this.isAI) {
        ...
        this.aiCooldown -= 1;
        left = dist < -70; right = dist > 70;
        guard = Math.random() < .006 && Math.abs(dist) < 110;
        if (Math.abs(dist) < 85 && this.aiCooldown <= 0) { light = Math.random() < .6; heavy = !light && Math.random() < .7; this.aiCooldown = 28 + Math.random()*28; }
        if (Math.abs(dist) < 180 && this.meter > 35 && this.aiCooldown <= 0 && Math.random() < .1) { special = true; this.aiCooldown = 70; }
        if (Math.random() < .01 && this.onGround) jump = true;
      } else {
        const k = state.keys;
      ...
      const moveSpeed = this.c.speed * (this.guard ? .45 : 1);
      this.vx = 0;
  ...
  function startFight(item, mode='story') {
    ...
    const p1 = new Fighter(playerId, 175, 1, { left:'a', right:'d', jump:'w', light:'j', heavy:'k', special:'l', guard:'s' }, false);
    const p2 = new Fighter(enemyId, 720, -1, { left:'arrowleft', right:'arrowright', jump:'arrowup', light:'1', heavy:'2', special:'3', guard:'arrowdown' }, isStory || isPvPAI ? true : false);
    if (isTraining) {
      p1.meter = p1.maxMeter;
      p2.hp = p2.maxHp;
      p2.meter = 0;
    }
    state.fight = { p1, p2, stage: item.stage || 'forest', title: item.title || 'Fight', chapter: item.chapter || (isPvPLocal || isPvPAI ? 'PVP MODE' : isTraining ? 'TRAINING MODE' : 'STORY FIGHT'), timer: 0, over: false, pvp: isPvPLocal, mode: fightMode, training: isTraining };
    ...
    if (modeHint) {
      modeHint.textContent = isTraining ? 'TRAINING: infinite health/meter · R reset' : isPvPAI ? 'AI OPPONENT: R rematch' : isPvPLocal ? 'LOCAL VERSUS: R rematch' : '';
...
${item.intro || item.title || 'Ready?'}`, isTraining ? 2200 : 1800);
  ...
  function loop() {
    ...
    const modeText = f.training ? 'Training mode: infinite health and meter. Press R to reset spacing.' : f.mode === 'pvp-ai' ? 'PvP vs AI: defeat the CPU rival. Press R for a rematch.' : f.pvp ? 'Local PvP: Player 1 versus Player 2. Press R for a rematch.' : 'Story battle: defeat the opponent to advance.';
  ...
  renderRoster(); updateSelectedPanel(state.selected); renderGallery();
})();
